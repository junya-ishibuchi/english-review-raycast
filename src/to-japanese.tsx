...
function saveRecord(input: string, output: string) {
  ...
  analyzeInBackground("toJapanese", input, output)
    .then((analysis) => {
      const record: LearningRecord = { id, type: "toJapanese", input, output, analysis, created_at, review_count: 0 };
      appendRecord(record).catch(console.error);
      updateCategories(analysis.categories).catch(console.error);
    })
    .catch(() => {
      const fallback: LearningRecord = { id, type: "toJapanese", input, output, analysis: { sentences: [], categories: [], difficulty: "basic" }, created_at, review_count: 0 };
